...
export default function SettingsPage(): JSX.Element {
  ...
  //
  // 2026-04-24 fix（第二次，補寫 orphan commit 9fad7c3d 未合入 main 的修復）：
  // 原先打 /auth/guest/upgrade，但該 endpoint 在 Phase A (3018a9c4) 重構時一併
  // 被砍掉，訪客點綁 Google 一律 404 → "upgrade failed: 404" toast。現對齊新
  // 架構「OAuth 為主要登入路徑」：popup 拿 Google idToken → POST
  // /auth/oauth/login/google → server 以 gmail 為識別碼自動建帳 / 登入 → 回
  // 站上 JWT → 用新 JWT stash+reload 重建 socket（provider='google'）。
  //
