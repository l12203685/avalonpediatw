...
/**
 * Quarantine toggle for the 5-player full-game test.
 *
 * The full-round test is network-timing-sensitive: it paces `game:vote`
 * emissions against the server's 1-vote-per-second rate limiter and waits on
 * multi-stage broadcast propagation. If CI ever shows flakiness on this one
 * test, set `AVALON_QUARANTINE_5P=1` in the CI env to skip it until the root
 * cause is diagnosed — without quarantining the 3 faster smoke tests in the
 * same file. Default (unset) is PASS / fully enforced.
 */
const QUARANTINE_5P_FULL = process.env.AVALON_QUARANTINE_5P === '1';

describe('M1 smoke: 5-player FULL 5-round Avalon game end-to-end', () => {
  let httpServer: HttpServer;
  let io: SocketIOServer;
  let gameServer: GameServer;
  let port: number;
  ...
  (QUARANTINE_5P_FULL ? it.skip : it)(
    'rounds 1-5 with mixed quest outcomes (3 success / 2 fail) -> assassination_failed (good wins) broadcast reaches all 5 players',
    async () => {
      // ── 1. Connect 5 guest clients ───────────────────────────────────────────
      const names = ['Alice', 'Bob', 'Charlie', 'David', 'Eve'];
      for (let i = 0; i < names.length; i++) {
        const uid = `full-uid-${i + 1}`;
        clients.push(await connectGuestClient(port, uid, names[i]));
      }
      expect(clients).toHaveLength(5);

      // ── 2. Host creates room + other 4 join + start ──────────────────────────
      const host = clients[0];
      host.socket.emit('game:create-room', host.displayName);
      ...
      // Canonical 5-player roles assigned.
      const assignedRoles: Role[] = clients.map((c) => c.startedState!.players[c.uid].role as Role);
      expect([...assignedRoles].sort()).toEqual([...AVALON_CONFIG[5].roles].sort());

      // ── 3. Collect role map so we can control quest outcomes deterministically.
      const roleByUid = new Map<string, Role>();
      for (const c of clients) {
        roleByUid.set(c.uid, c.startedState!.players[c.uid].role as Role);
      }
      ...
      const merlinUid = [...roleByUid.entries()].find(([, r]) => r === 'merlin')![0];
      const assassinUid = [...roleByUid.entries()].find(([, r]) => r === 'assassin')![0];
      expect(goodUids).toHaveLength(3); // 1 merlin + 1 percival + 1 loyal
      expect(evilUids).toHaveLength(2); // 1 assassin + 1 morgana
      expect(merlinUid).toBeTruthy();
      expect(assassinUid).toBeTruthy();
      ...
      /**
       * Drive one full round: leader selects a team, everyone approves, then
       * team members submit quest votes. Returns when the round is resolved
       * (room transitions to next voting round, or to 'discussion' for round 5).
       *
       * `forceFail` places the assassin on the team and has the assassin vote
       * FAIL, guaranteeing quest result = fail. Otherwise picks a good-only
       * team and everyone votes success.
       */
      async function driveRound(roundIdx: number, forceFail: boolean, isLast: boolean): Promise<void> {
        const roundNum = roundIdx + 1;
        // 1) Wait for voting ready state.
        await waitFor(
          () => {
            const r = host.latestState;
            if (!r) return null;
        ...
        const currentRoom = host.latestState!;
        const teamSize = AVALON_CONFIG[5].questTeams[roundIdx];
        const leaderId = orderedPlayerIds[currentRoom.leaderIndex % orderedPlayerIds.length];

        // Team composition: forceFail -> 1 evil (assassin) + fill with good;
        ...
        // 2) Approve-vote: all 5 approve. Pace 1.1s between rounds to stay
        //    above the server's 1-vote-per-second per-socket limiter floor.
        ...
        // 3) Quest votes: good players always vote success; evil on team votes
        //    fail iff forceFail (otherwise success so team all-good guarantees
        //    success anyway). Quest votes have no rate limiter.
        for (const uid of team) {
          const c = clients.find((cl) => cl.uid === uid)!;
        ...
        // 4) Wait for round resolution.
        await waitFor(
          () => {
            const r = host.latestState;
            if (!r) return null;
            if (isLast) return r.state === 'discussion' ? true : null;
            // Non-terminal rounds: advance to the next voting round.
      ...
      // ── 4. Drive all 5 rounds: S, F, S, F, S -> good hits 3 successes on R5.
      //     (5-player rules: 1 fail vote required per round for quest fail.)
      ...
      await driveRound(4, false, true);  // R5 success -> discussion (good 3-2)

      // ── 5. Verify mixed quest results reached all clients.
      const roomAtDiscussion = host.latestState!;
      expect(roomAtDiscussion.state).toBe('discussion');
      ...
      // ── 6. Assassin misses Merlin -> GOOD wins. Pick any non-merlin,
      //     non-assassin target (e.g. a loyal or percival).
      ...
      await waitFor(
        () => clients.every((c) => c.endedState !== null),
        { label: 'all 5 players receive game:ended (good wins)', timeoutMs: 5000 }
      );

      // ── 7. Final assertions: good wins, all roles revealed to every client.
      for (const c of clients) {
        const ended = c.endedState!;
        expect(ended.state).toBe('ended');
      ...
      // No unexpected errors on any socket.
      for (const c of clients) {
        expect(c.errors).toEqual([]);
      }
    },
