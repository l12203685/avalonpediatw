...
import { ArrowLeft, ChevronRight, ChevronLeft, Users, ThumbsUp, ThumbsDown, CheckCircle, XCircle, Swords, Trophy } from 'lucide-react';
import { useGameStore } from '../store/gameStore';
import { MOCK_REPLAY, ROLE_DISPLAY, ReplayData, ReplayEvent } from '../data/mockData';

// ─── Event Card ───────────────────────────────────────────────────────────────

function EventCard({ event, visible }: { event: ReplayEvent; visible: boolean }): JSX.Element {
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          {event.team?.map((name) => (
            <span key={name} className="text-xs bg-purple-800/50 text-purple-200 border border-purple-600/40 px-2 py-1 rounded-full">
              {name}
            </span>
          ))}
        </div>
  ...
  if (event.type === 'quest-result') {
    ...
    return (
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: visible ? 1 : 0.2, y: 0 }}
        className={`${base} ${success ? 'bg-green-900/25 border-green-600/50' : 'bg-red-900/25 border-red-600/50'}`}
      >
        <div className="flex items-center gap-2">
          {success
    ...
    return (
      ...
      >
        ...
        <p className="text-white text-sm">
          ...
          {' 刺殺了 '}
          <span className={`font-semibold ${event.targetWasMerlin ? 'text-red-400' : 'text-gray-300'}`}>
            {event.target}
          </span>
        </p>
        <p className={`text-sm font-semibold ${event.targetWasMerlin ? 'text-red-400' : 'text-green-400'}`}>
          {event.targetWasMerlin ? '✓ 暗殺成功！目標是 Merlin' : '✗ 暗殺失敗！目標不是 Merlin'}
        </p>
      </motion.div>
    );
  }
  ...
  if (event.type === 'game-end') {
    ...
    return (
      <motion.div
        ...
        className={`${base} ${good ? 'bg-green-900/30 border-green-500' : 'bg-red-900/30 border-red-500'} text-center`}
      >
...
function RoundTrack({ results }: { results: ReplayData['questResults'] }): JSX.Element {
  return (
    <div className="flex items-center justify-center gap-3">
      ...
      {Array.from({ length: 5 - results.length }).map((_, i) => (
        <div key={`empty-${i}`} className="w-8 h-8 rounded-full border-2 border-dashed border-gray-600" />
      ))}
    </div>
  );
}
...
function PlayerRoster({ players }: { players: ReplayData['players'] }): JSX.Element {
  return (
...
export default function ReplayPage(): JSX.Element {
  ...
  const replay = MOCK_REPLAY; // TODO: fetch from /api/replay/:replayRoomId
  ...
  const ago = (() => {
    const d = Math.floor((Date.now() - replay.playedAt) / 86400000);
    const h = Math.floor((Date.now() - replay.playedAt) / 3600000);
    if (d > 0) return `${d} 天前`;
    return `${h} 小時前`;
  })();
    ...
    <div className="min-h-screen bg-gradient-to-b from-avalon-dark to-black">
      ...
      >
        <div className="max-w-2xl mx-auto text-center space-y-3">
          ...
          <p className={`text-lg font-bold ${replay.winner === 'good' ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Events — show all up to current step */}
        ...
        <p className="text-center text-xs text-gray-600 pb-4">
          資料來源：/api/replay/:roomId（目前顯示模擬資料）
        </p>
      </div>
    </div>
  );
