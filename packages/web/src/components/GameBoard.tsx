...
export default function GameBoard({
  ...
  return (
    ...
    >
      ...
      {/* Center — state banner + children (quest/vote/history) + chat/scoresheet slots */}
      <section className="flex flex-col gap-2 sm:gap-3 min-w-0 min-h-0 overflow-y-auto">
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="shrink-0 text-center bg-gradient-to-b from-avalon-card/60 to-avalon-card/30 border border-gray-700/60 rounded-lg sm:rounded-xl py-2 px-3 sm:py-3 sm:px-4"
        >
          <motion.p
            key={room.state}
          ...
          {room.state === 'voting' && (
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mt-0.5 sm:mt-1 text-[11px] sm:text-xs text-gray-400"
            >
