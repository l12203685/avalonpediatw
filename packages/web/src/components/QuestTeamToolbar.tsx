...
export default function QuestTeamToolbar({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 40 }}
      // Edward 2026-04-27 08:01 root-fix: 砍 `fixed bottom-0` + 父層 `pb-[30dvh]`
      // 預留 (詳見 VotePanel 同段註解). 改 `flex-none` 進 normal flow.
      className="relative w-full flex-none max-h-[40dvh] overflow-y-auto bg-gradient-to-t from-black/95 via-black/90 to-black/75 backdrop-blur-md border-t-2 border-amber-500 shadow-[0_-6px_20px_rgba(0,0,0,0.55)] pb-safe"
      role="region"
